...
/* util simpel untuk balikan json / redirect back */
    ...
    public function show(Order $order)
    {
        ...
        $role = strtolower($order->user->role ?? auth()->user()->role);

        // kalau corporate (atau typo coorporate), lempar ke halaman completion
        if (in_array($role, ['corporate','coorporate'])) {
            // opsional: pastikan transaksi full dibuat untuk corporate
            $this->makeTx($order, true);

            return redirect()
                ->route('payment.complete', $order)
                ->with('info', 'Akun corporate diarahkan ke halaman upload PO / bukti.');
        }
    ...
    public function submitCompletion(Request $r, Order $order)
    {
        ...
        $path = $r->file('transfer_proof_full')->store('transfer_proofs','private');
